...
ErrorOr<u8*> mmap(size_t size, int prot, int flags, int fd, off_t offset)
{
    return TRY(mmap(nullptr, size, prot, flags, fd, offset));
}

ErrorOr<u8*> mmap(void* addr, size_t size, int prot, int flags, int fd, off_t offset)
{
...
{
    if ((flags & O_CREAT) != 0) {
        return Error::from_string_literal("O_CREAT should not be used with this function variant");
    }
...
{
    pid_t pid = -1;
    auto rc = ::posix_spawnp(&pid, file, file_actions, attrp, (char**)argv, (char**)envp);
    if (rc != 0)
